...
public partial class InventorySimulator
{
    ...
    )
    {
        ...
        // Verificar se pode aplicar mudanças imediatamente
        var canApplyImmediately = ConVars.IsWsImmediately.Value;
        
        // Se troca instantânea for exclusiva para VIP, verificar status
        if (canApplyImmediately && ConVars.IsWsImmediatelyVipOnly.Value)
        {
            canApplyImmediately = player.GetState().IsVip();
        }
        
        if (inventory != null && canApplyImmediately)
        {
    ...
    )
    {
        ...
        var isVip = player.GetState().IsVip();
        var item = ItemHelper.IsMeleeDesignerName(designerName)
            ? inventory?.GetKnife(player.TeamNum, isFallbackTeam, isVip)
            : inventory?.GetWeapon(
                player.TeamNum,
