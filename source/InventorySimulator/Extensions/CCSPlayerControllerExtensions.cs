...
public static class CCSPlayerControllerExtensions
{
    ...
    )
    {
        ...
        var teamNum = self.TeamNum;
        var isVip = self.GetState().IsVip();
        var item = inventory.GetGloves(teamNum, isFallbackTeam, isVip);
        var oldItem = oldInventory?.GetGloves(teamNum, isFallbackTeam, isVip);
        
        // Se ambos são null mas o jogador não é VIP, ainda precisamos atualizar
        // para garantir que luvas de VIP anterior sejam removidas
        if (oldItem == item && (item != null || isVip))
            return;
            ...
            )
            {
                ...
                var oldItem =
                    data.GearSlot is gear_slot_t.GEAR_SLOT_KNIFE
                        ? oldInventory?.GetKnife(self.TeamNum, isFallbackTeam, isVip)
                        : oldInventory?.GetWeapon(self.TeamNum, entityDef, isFallbackTeam);
                var item =
                    data.GearSlot is gear_slot_t.GEAR_SLOT_KNIFE
                        ? inventory.GetKnife(self.TeamNum, isFallbackTeam, isVip)
